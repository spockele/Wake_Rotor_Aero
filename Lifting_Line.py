...
class HorseShoe:
    ...
    def GenerateWakePoints(self, axialVelocity: float, rotSpeed: float, resolution: int, tmax: float):
        ...
        for t in intervals:
            ...
            if not t:
                z = 0 + self.pos_inner.refPos[2]
                y_inner = self.pos_inner.rloc * np.sin(self.pos_inner.thetaloc) + self.pos_inner.refPos[1]
                x_inner = self.pos_inner.rloc * np.cos(self.pos_inner.thetaloc) + self.pos_inner.refPos[0]
                pos_inner_trail = Vec((x_inner, y_inner, z))
                inner_leg_control_points.append(pos_inner_trail)

                y_outer = self.pos_outer.rloc * np.sin(self.pos_outer.thetaloc) + self.pos_inner.refPos[1]
                x_outer = self.pos_outer.rloc * np.cos(self.pos_outer.thetaloc) + self.pos_inner.refPos[0]
