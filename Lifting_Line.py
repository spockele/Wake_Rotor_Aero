...
class HorseShoe:
    ...
    def GenerateWakePoints(self, axialVelocity: float, rotSpeed: float, resolution: int, tmax: float):
        ...
        for t in intervals:
            ...
            z = t*axialVelocity + self.pos_inner.refPos[2]

            # Every horseshoe has a pos_inner and pos_outer, so do the calculation twice
            y_inner = self.pos_inner.rloc * np.sin(rotSpeed * t + self.pos_inner.thetaloc) + self.pos_inner.refPos[1]
            x_inner = self.pos_inner.rloc * np.cos(rotSpeed * t + self.pos_inner.thetaloc) + self.pos_inner.refPos[0]
            pos_inner_trail = Vec((x_inner, y_inner, z))
            inner_leg_control_points.append(pos_inner_trail)

            y_outer = self.pos_outer.rloc * np.sin(rotSpeed * t + self.pos_outer.thetaloc) + self.pos_inner.refPos[1]
            x_outer = self.pos_outer.rloc * np.cos(rotSpeed * t + self.pos_outer.thetaloc) + self.pos_inner.refPos[0]
